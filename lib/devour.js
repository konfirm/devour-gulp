...
function Devour(settings) {
	...
	function preload() {
		...
		if ('defaults' in config) {
			register('pipe', 'devour_defaults', function(stream, devour) {
			...
			if (config.verbose) {
				console.log(
					'  - created pipe "%s" for plugins: %s',
					chalk.cyan('devour_defaults'),
					chalk.green(config.defaults.join(chalk.white(' > ')))
				);
			}
		}
	...
	function startRequested() {
		...
		if (run.length) {
			run.forEach(function(task) {
				var exists = task in definitions.task;
	...
	devour.source = function() {
		return gulp.src.apply(gulp, arguments)
			.pipe(devour.pipe('devour_defaults'))
		;
	};
